...
class MainActivity : AppCompatActivity() {
    ...
    val updateUI = Runnable {
        runOnUiThread(Runnable {
            colorName1TextView.setText(color1Name)
            colorName2TextView.setText(color2Name)
            colorName3TextView.setText(color3Name)
    ...
    override fun onStart() {
        ...
        successSound.setOnPreparedListener(MediaPlayer.OnPreparedListener { mediaPlayer ->  mediaPlayer.start(); mediaPlayer.setVolume(0.0f, 0.0f)})
    ...
    private fun calculateColorDistance(r1:String, g1:String, b1:String, r2:String, g2:String, b2:String): Float {
        return Math.sqrt(Math.pow(r2.toDouble() - r1.toDouble(), 2.0)
                + Math.pow(g2.toDouble() - g1.toDouble(), 2.0)
                + Math.pow(b2.toDouble() - b1.toDouble(), 2.0)).toFloat()
    }

    private fun calculateMinColorDistance(): Float {
       // val rgbColors1 = firstColor!!.split("-")
        val rgbColors2 = currentColor.split("-")
        var distance = 999.0f//calculateColorDistance(rgbColors1[0], rgbColors1[1], rgbColors1[2], rgbColors2[0], rgbColors2[1], rgbColors2[2])
